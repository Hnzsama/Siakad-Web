...
export function DataTableToolbar<TData>({
    ...
    const { teacherFilter, classroomFilter, subjectFilter } = Data()
    const [selectedIds, setSelectedIds] = React.useState<string[]>([])

    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as GroupedClassSubject).id)
                ...
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as GroupedClassSubject

                                const currentRow: GroupedClassSubject = {
                                    ...selectedRow,
                                    selectedIds: selectedIds,
                                    classroom: {
                                        ...selectedRow.classroom,
