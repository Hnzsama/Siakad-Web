...
export function DataTableToolbar<TData>({
    ...
    const { semesterFilter, classroomFilter } = Data()
    const [selectedIds, setSelectedIds] = React.useState<string[]>([])

    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as StudentHistory).id)
        ...
        <div className='flex items-center justify-between'>
            <div className='flex flex-col-reverse items-start flex-1 gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as StudentHistory

                                const currentRow: StudentHistory = {
                                    ...selectedRow,
                                    selectedIds: selectedIds,
                                    id: `${selectedIds.length} selected semester`
                                }
                ...
                <div className='flex gap-x-2'>
                    {table.getColumn('semester_id') && (
                        <DataTableFacetedFilter
                            column={table.getColumn('semester_id')}
                            title='Semester'
                            options={semesterFilter.map((t) => ({ ...t }))}
                        />
                    )}
                    {table.getColumn('classroom_id') && (
                        <DataTableFacetedFilter
