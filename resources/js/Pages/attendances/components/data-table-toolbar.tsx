...
export function DataTableToolbar<TData>({
    ...
    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as Attendance).id)
        ...
        <div className='flex items-center justify-between'>
            <div className='flex flex-col-reverse items-start flex-1 gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as Attendance

                                const currentRow: Attendance = {
                                    ...selectedRow,
                                    selectedIds: selectedIds,
                                    attendable: {
                                        ...selectedRow.attendable,
                ...
                <div className='flex gap-x-2'>
                    {table.getColumn('attendable_type') && (
                        <DataTableFacetedFilter
                            column={table.getColumn('attendable_type')}
                            title='Jenis'
