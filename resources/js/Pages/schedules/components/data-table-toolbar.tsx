...
export function DataTableToolbar<TData>({
    ...
    const { shiftFilter, statusFilter } = Data()
    const [selectedIds, setSelectedIds] = React.useState<string[]>([])

    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as Schedule).id)
                ...
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as Schedule

                                const currentRow: Schedule = {
                                    ...selectedRow,
                                    selectedIds: selectedIds,
                                    day: `${selectedIds.length} jadwal terpilih` // Menggunakan bahasa Indonesia
                                }
