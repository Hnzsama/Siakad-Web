...
export function DataTableToolbar<TData>({
    ...
    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as ClassLevel).id)
                ...
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as ClassLevel

                                const currentRow: ClassLevel = {
                                    ...selectedRow,
                                    selectedIds: selectedIds,
                                    alphabet: `${selectedIds.length} selected semester`
                                }
