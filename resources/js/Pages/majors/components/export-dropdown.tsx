...
import { Major, Exportable } from "../data/schema";
...
export function ExportDropdown({ data }: ExportDropdownProps) {
    ...
    const exportToCsv = () => {
        ...
        downloadFile(csvContent, 'majors.csv', 'text/csv');
    };
    ...
    const exportToSql = () => {
        ...
        const sqlStatements = exportData.map(data => {
            ...
            return `INSERT INTO majors (${columns.join(', ')}) VALUES (${values.join(', ')});`;
        }).join('\n');

        downloadFile(sqlStatements, 'majors.sql', 'text/plain');
    };
