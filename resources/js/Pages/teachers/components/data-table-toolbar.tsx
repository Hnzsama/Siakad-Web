...
export function DataTableToolbar<TData>({
  ...
  const { genderFilter, statusFilter, shiftFilter } = Data()
  const [selectedIds, setSelectedIds] = React.useState<string[]>([])

  React.useEffect(() => {
    const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as Teacher).id)
            ...
            <div className='flex space-x-2'>
              ...
              {Object.keys(rowSelection).length > 0 && (
                <Button
                  ...
                  onClick={() => {
                    const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as Teacher

                    const currentRow: Teacher = {
                      ...selectedRow,
