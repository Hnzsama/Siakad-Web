...
export function DataTableToolbar<TData>({
    ...
    const { statusFilter, violationTypeFilter } = Data()
    const [selectedIds, setSelectedIds] = React.useState<string[]>([])

    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as Violation).id)
        ...
        <div className='flex items-center justify-between'>
            <div className='flex flex-col-reverse items-start flex-1 gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as Violation

                                const currentRow: Violation = {
                                    ...selectedRow,
                ...
                <div className='flex gap-x-2'>
                    {table.getColumn('violation_type_id') && (
                        <DataTableFacetedFilter
                            column={table.getColumn('violation_type_id')}
                            title='Jenis'
                            options={violationTypeFilter.map((t) => ({ ...t }))}
                        />
                    )}
