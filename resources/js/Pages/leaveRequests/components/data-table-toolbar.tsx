...
export function DataTableToolbar<TData>({
    ...
    const { typeFilter, statusFilter } = Data()
    const [selectedIds, setSelectedIds] = React.useState<string[]>([])

    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as LeaveRequest).id)
        ...
        <div className='flex items-center justify-between'>
            <div className='flex flex-col-reverse items-start flex-1 gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as LeaveRequest

                                const currentRow: LeaveRequest = {
                                    ...selectedRow,
                                    selectedIds: selectedIds,
                                    leavable: {
                                        ...selectedRow.leavable,
                ...
                <div className='flex gap-x-2'>
                    {table.getColumn('leavable_type') && (
                        <DataTableFacetedFilter
                            column={table.getColumn('leavable_type')}
                            title='Jenis'
                            options={typeFilter.map((t) => ({ ...t }))}
                        />
                    )}
