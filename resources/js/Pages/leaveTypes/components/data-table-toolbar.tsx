...
export function DataTableToolbar<TData>({
    ...
    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as LeaveType).id)
                ...
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as LeaveType

                                const currentRow: LeaveType = {
                                    ...selectedRow,
                                    selectedIds: selectedIds,
                                    leave_name: `${selectedIds.length} selected semester`
                                }
