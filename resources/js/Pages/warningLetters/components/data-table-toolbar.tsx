...
export function DataTableToolbar<TData>({
    ...
    React.useEffect(() => {
        const selectedIds = table.getFilteredSelectedRowModel().rows.map(row => (row.original as WarningLetter).id)
                ...
                <div className='flex space-x-2'>
                    ...
                    {Object.keys(rowSelection).length > 0 && (
                        <Button
                            ...
                            onClick={() => {
                                const selectedRow = table.getFilteredSelectedRowModel().rows[0]?.original as WarningLetter

                                const currentRow: WarningLetter = {
                                    ...selectedRow,
                                    selectedIds: selectedIds,
                                    letter_number: `${selectedIds.length} selected semester`
                                }
