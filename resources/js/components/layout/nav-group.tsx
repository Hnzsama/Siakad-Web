...
function checkIsActive(href: string, item: NavItem, mainNav = false) {
    // Get base paths without query parameters
    const currentPath = href.split('?')[0]
    const itemPath = item.url ? item.url.split('?')[0] : ''
    ...
    if (item?.items?.some(i => {
        const childPath = i.url.split('?')[0]
        return href === i.url || currentPath.startsWith(childPath)
    })) return true

    // Check main navigation sections
    if (mainNav &&
        href.split('/')[1] !== '' &&
        currentPath.split('/')[1] === itemPath.split('/')[1]) return true
