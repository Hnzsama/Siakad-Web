    ...
    public function rules(): array
    {
        return [
            ...
            'nik' => [
                ...
                'string',
                'size:16',
                Rule::unique(Student::class, 'nik')->ignore($this->student->id),
            ...
            'phone' => [
                ...
                Rule::unique(Student::class, 'phone')->ignore($this->student->id),
                Rule::unique(Guardian::class, 'phone'),
                Rule::unique(User::class, 'phone')->ignore($this->student->user_id),
            ],
            'email' => [
                'required',
                'email',
                Rule::unique(Teacher::class, 'email'),
                Rule::unique(Student::class, 'email')->ignore($this->student->id),
                Rule::unique(Guardian::class, 'email'),
                Rule::unique(User::class, 'email')->ignore($this->student->user_id),
            ],
            'status' => [
                'required',
                'string',
